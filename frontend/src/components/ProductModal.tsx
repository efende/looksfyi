import { ChevronLeft, Edit2, Upload, Download } from 'lucide-react';
            ...
            <div className={`relative bg-white rounded-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 ${isLook ? 'w-[90vw] max-w-6xl h-[85vh] flex' : 'w-fit max-w-[90vw]'}`}>
                ...
                {isLook && (
                    <div className="w-1/3 h-full flex flex-col border-l border-gray-100 shrink-0 relative overflow-hidden bg-white">
                                ...
                                <div className="flex-1 overflow-y-auto px-6 py-6 transition-all">
                                    ...
                                    <div className="space-y-4">
                                        {product.relatedProducts.map((item, idx) => (
                                            <div
                                                key={idx}
                                                className="flex gap-4 p-2 rounded-xl hover:bg-gray-50 transition-colors border border-gray-100 bg-white group/item"
                        ...
                        {/* 2. Slide-in Detail View (Overlay) */}
                        <div
                            className={`absolute inset-0 bg-white z-20 flex flex-col transition-transform duration-300 ease-in-out ${selectedRelatedItem ? 'translate-x-0' : 'translate-x-full'
                                }`}
                        >
                            {selectedRelatedItem && (
                                <>
                                    ...
                                    <div className="p-6 bg-white shrink-0 border-t border-gray-100">
                                        <p className="text-xs text-gray-500 font-medium mb-1">{selectedRelatedItem.brand}</p>
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="text-lg font-semibold text-gray-900 leading-tight">{selectedRelatedItem.name}</h3>
                                            <span className="text-lg font-medium text-gray-900">{selectedRelatedItem.price}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between pt-2">
                                            <button className="relative overflow-hidden group/btn bg-gradient-to-r from-[#E2E2E2] via-[#E2E2E2] to-[#E2E2E2] text-black px-4 py-2 rounded-full text-xs font-semibold shadow-lg transition-all hover:scale-105 active:scale-95">
                                                <div className="absolute inset-0 bg-[linear-gradient(90deg,transparent_45%,rgba(255,0,128,0.3)_75%,rgba(121,40,202,0.3)_100%)] opacity-70 group-hover/btn:opacity-100 transition-opacity"></div>
                                                <div className="absolute inset-0 translate-x-[-100%] group-hover/btn:translate-x-[100%] transition-transform duration-1000 bg-gradient-to-r from-transparent via-white/50 to-transparent"></div>
                                                <span className="relative z-10 flex items-center gap-2">AI Try-On</span>
                                            </button>
                                            <div className="flex gap-2">
                                                <button className="bg-gray-100 text-black px-4 py-2 rounded-full text-xs font-medium hover:bg-gray-200 transition-colors">
                                                    Save
                                                </button>
                                                <button className="bg-gray-100 text-black px-4 py-2 rounded-full text-xs font-medium hover:bg-gray-200 transition-colors">
                                                    Buy
                                            ...
                                            <div className="flex gap-1">
                                                <button className="p-1.5 text-gray-600 hover:text-black hover:bg-gray-50 rounded-full transition-colors">
                                                    <Edit2 size={16} />
                                                </button>
                                                <button className="p-1.5 text-gray-600 hover:text-black hover:bg-gray-50 rounded-full transition-colors">
                                                    <Upload size={16} />
